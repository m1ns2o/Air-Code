...
public final class AirCodeAPI: Sendable {
    ...
    public func startAgent(projectId: String, agent: String, prompt: String, mode: AgentMode, provider: String, model: String, reasoningEffort: ReasoningEffort, resumeSession: Bool, caveman: Bool) async throws -> StartAgentResponse {
        try await send(path: "/v1/projects/\(projectId)/agents/runs", method: "POST", body: StartAgentRequest(agent: agent, prompt: prompt, mode: mode, provider: provider, model: model, reasoningEffort: reasoningEffort, resumeSession: resumeSession, caveman: caveman))
    }
