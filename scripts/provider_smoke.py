...
def live_agent_smoke(base_url, token, agent_id):
    ...
    time.sleep(3)
    log = request(base_url, token, "GET", f"/v1/projects/sample-app/agents/runs/{run_id}/log", timeout=30)
    changes = request(base_url, token, "GET", f"/v1/projects/sample-app/agents/runs/{run_id}/changes", timeout=30)
    revert = request(base_url, token, "POST", f"/v1/projects/sample-app/agents/runs/{run_id}/revert", timeout=30)
    stop = request(base_url, token, "POST", f"/v1/projects/sample-app/agents/runs/{run_id}/stop", timeout=30)
    resume = request(
        base_url,
        token,
        "POST",
    ...
    time.sleep(1)
    request(base_url, token, "POST", f"/v1/projects/sample-app/agents/runs/{resume['runId']}/stop", timeout=30)
    return {
        "runId": run_id,
        "steerAccepted": steer.get("accepted", False),
        "logBytes": len(log.get("content", "")),
        "changes": len(changes.get("changes", [])),
        "reverted": len(revert.get("reverted", [])),
        "stop": stop,
    ...
    try:
        ...
        for agent_id in ("codex", "hermes", "claude"):
            ...
            if not cap.get("installed") or not cap.get("configured"):
                ...
                try:
                    entry["live"] = live_agent_smoke(base_url, token, agent_id)
                    entry["status"] = "passed"
